...
Since M8 the gate is the pure `gate_reason(request)`, which returns the first failing check of six: `public_api_host_unconfigured`, `host_absent`, `host_mismatch`, `origin_verify_secret_empty`, `origin_verify_absent`, `origin_verify_mismatch`. Each rejection logs one `app.gate` WARNING through `app/gate_logging.py` (`event="host_gate"`, `outcome="rejected"`, the `reason`, the `ip` described next, the sanitized `request_id`, the path truncated to 128, and the `Host` / `X-Origin-Verify` headers' presence and length only). `ip` is `Fly-Client-IP` (the last copy), the address Fly's proxy accepted the connection from: the sender's own on a direct request, a Cloudflare edge address otherwise. It is never `CF-Connecting-IP` or an `X-Forwarded-For` entry: `CF-Connecting-IP` and the first entry are attacker-controlled on a request that went around the edge, and the last entry is Fly's own edge, the app's anycast address, the same on every request. Without `Fly-Client-IP` it is the socket peer when there is no `X-Forwarded-For` either, otherwise `"unknown"`, because uvicorn's `--proxy-headers --forwarded-allow-ips=*` sets `request.client.host` to the first entry. Whether Fly overwrites a client-sent `Fly-Client-IP` is unproven until the next release (TODOS.md). Those fields are the log message itself, one JSON object (`{"event": "host_gate", …}`), because the app installs no logging config and under uvicorn the logger reaches Python's last-resort handler, which prints the message alone; they also ride on the record as `extra`. The emitter is called inside a `try`, so a logging failure never changes the 421, whose body stays byte-identical across all six reasons. `GATE_BREAK_GLASS` (`fly.toml [env]`, default `"0"`; only the exact string `"1"` enables it) skips the origin-verify checks while the Host checks still run, and logs every admitted request as `outcome="bypassed"`; `/healthz` reports it. The procedure is `infra/incident-diagnostics.md` → Break-glass.
