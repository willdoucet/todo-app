...
- Before calling something a blocker, name the step that would fail and check that the step actually reads it. A failing signal is not a failing gate: M8's paused worker changes what the smoke checks and the Settings card report, but neither a merge nor `fly deploy` reads the worker (corrected 2026-09-23).
...
- "Where they sit" includes the first execution. A step that names "the previous release tag" needs a line for the release that has none, and a rollback that checks out the previous commit cannot then run a script that commit does not have. M8 PR1a's RUNBOOK had both, on the one release that was scheduled to dry-run the rollback (found 2026-09-23 by `/final-review`).
- Name "the previous release" with something that survives a release of several merges, and treat a vendor behavior read from its docs as an assumption until the first run. M8's RUNBOOK used `$RELEASE^` before the first tag, but the PR1a release spanned #60 and #61, so `^` named PR1a itself; a tag per release is the reliable name. The same run found Vercel's documented "a promoted deployment cannot be promoted again; use Instant Rollback" backwards on the Hobby plan: Promote worked, and Instant Rollback refused anything older than the previous production deployment (402). Found 2026-09-23 at the first executed release.
...
- A check quotes behavior, not a value that moves with the base branch. `workflow-rereview-utc`'s success criterion said `design-sync-check --pin-sha` "still prints `c0f7f30…`"; the pin is the last commit touching a watched file, so a docs-only pull request (#55) that edited `FRONTEND_GUIDELINES.md` made a true sentence false in three places (plan, criterion, test artifact) one day after it was verified. Write "exits 0 and prints a pin" as the check, and put the value seen in the execution summary, where a dated observation belongs. Found 2026-09-20 (UTC) by `/final-review` of `workflow-rereview-utc`.
...
- The reset can land **mid-command** when another shell call runs in parallel: a `cd <worktree> && …` chain started beside a 70-second test run had its later steps execute in the main checkout, and `doc-guard` reported `branch=prod-launch-release` with exit 0. Never run a worktree command in parallel with another shell call. Open every command whose output you will trust, and every state-writing one, with a branch assertion that aborts (`[ "$(git branch --show-current)" = "<branch>" ] || exit 9`), wrap each helper in its own `(cd "$W" && …)` subshell, and read the branch name the helper prints before believing its verdict.
...
- The stash stack (`refs/stash`) is shared by every worktree of the repository, and other sessions push and pop it. From a worktree, do not take `/ship`'s `git stash push` / `git stash pop` fallback when `git merge` refuses over uncommitted doc rows: a pop can land another session's entry. Use the patch dance under Patterns That Work instead (save the diff, restore the file, merge, `git apply --3way`). Noted 2026-09-20 (UTC) by `/final-review` of `workflow-rereview-utc`, ahead of its `/ship` sync past #56.
...
- **Pydantic `Field(pattern=...)` is not Python's `re`.** pydantic-core compiles it with Rust's
  regex engine, which has no `\Z`: the model class fails to build at import (`SchemaError: regex
  parse error … unrecognized escape sequence`), and on a module `main.py` imports that is a web
  process that cannot boot. Use `\z` there (Rust's end-of-text anchor; Rust's `$` also does not
  admit a trailing newline). Found 2026-09-23 in M8 PR1b: the dev api's hot reload died on
  `app/job_health.py`'s timestamp pattern.
...
| 2026-09-14 | self | M8 design review wrote into the plan that a forced sign-in redirect shows "no explanation", trusting APP_FLOW §4's "silent one-shot redirect" note; `AuthPortalPage.jsx` has rendered a "Your session ended" bounce banner whenever `return_to` is present since M4 | Read the rendering component before describing what a user sees on a screen; a doc's description of UI behavior is a pointer, not evidence. The APP_FLOW note is stale |
...
| 2026-09-18 | self | `/review-implementation` ran the doc gate from the worktree in parallel with another shell call; the cwd reset landed mid-command and `doc-guard` graded the main checkout (`branch=prod-launch-release`, exit 0). Read-only, so nothing was damaged, and it was caught only because the helper prints the branch | Never parallelize worktree commands; assert the branch at the top and abort, one `(cd "$W" && …)` subshell per helper, and read the branch the helper prints (Worktrees rule above) |
| 2026-09-19 | user | `/final-review` of `workflow-rereview-utc` recommended deferring six small framework findings to the hardening issue, by precedent of the 20 deferred the day before; four of the six came in with the PR's own delta, and the user chose "fix upstream now" | A deferral precedent covers what it was decided for: pre-existing *and* many. Few and small, or caused by the PR under review, gets "fix upstream now" as the recommendation, whatever an earlier pass deferred; say which class each finding is in before recommending a route |
| 2026-09-23 | user | Told the user the Celery worker had to be running before M8 PR1b could merge and deploy; it only changes what the smoke checks and the Settings card report, and neither the merge nor `fly deploy` reads it | Name the step that would fail and check that it reads the thing claimed before calling it a blocker; a failing signal is not a failing gate |
| 2026-09-23 | self | PR #60's body and the plan's reason listed M8's operator steps in the wrong order (steps that need the merged code sat before the merge); caught and fixed before the user acted on them | Order operator steps by the state each one needs (before merge, after merge and before deploy, after deploy), and check each step against what it reads |
...
| 2026-09-14 | `.agents/skills/plan-design-review/SKILL.md` step 8E | `DESIGN_PIN_SHA=$(git log -1 --format=%H -- $WATCHED)` returned an **empty** pin SHA in the operator's zsh: zsh does not word-split an unquoted `$WATCHED`, so git received one path containing spaces, matched nothing, and exited 0. A plan would have been written with a blank design pin and downstream drift checks would compare against nothing | Computed the SHA with the three watched paths passed as separate arguments. `.agents/bin/design-sync-check --pin-sha` now builds the argument list in Python, never falls back to `HEAD`, and exits 2 with the reason on stderr when there is no SHA; plan-design-review, execute-plan, review-implementation, and design-review call it instead of shell. Tests in `.agents/tests/test_design_sync.py`; same change in the framework payload. A payload fix hand-applied to both repos leaves `.agents/manifest.json` stale, so `framework doctor` reports the files as modified locally: run `framework upgrade` after the framework change merges |
...
| 2026-09-17 | `.agents/bin/_lib.py` `review_log_append`, `review-log`, `review-read` (framework 1.1.0) | A plan review run on an **epic** branch and logged by the skill's own call stores `plan: null`: all three resolve "the plan" with `latest_plan`, which sees feature plans only, while `workflow-state` uses `find_current_plan` (feature plan, else the epic). The entry never counts, so `--next` keeps asking for the CEO review the epic just had. Reproduced in a temp repo with the framework's test helpers; todo-app's log holds no epic-named entry at all. Hidden because every epic test passes the plan name by hand. | Fixed in framework 1.2.0 (framework PR #6, `b17c320`; Eng review 1E of `workflow-rereview-utc`): the lookup moved to `_lib.current_plan` and all four callers use it; todo-app takes it with that plan's PR-T. Pinned by `test_epic_branch_reviews_attach_to_the_epic_and_can_be_demanded` and `test_current_plan_finds_the_feature_plan_then_the_epic`. On 1.1.0, pass `--plan <epic basename>` when logging a review on an epic. Found by `/plan-eng-review` while checking the plan's new demand plumbing against the code. |
| 2026-09-18 | `.agents/bin/workflow-state`, `review-read`, `_lib.print_json` (framework 1.2.0; the blocked-plan `reason` had the same exposure in 1.1.0) | `"\ud800"` is valid JSON, so a hand-appended or union-merged log line, a registry entry or a frontmatter value can hold a lone surrogate, and `print()` cannot encode it. The fail-silent session banner printed nothing at all, which the plan's own Eng review 1C calls an open gate, while `--next`, `--dashboard` and `review-read` died with `UnicodeEncodeError`. Claude Code's `--json` hook escapes it and was unaffected; the CLI could never write such a line, so only readers were exposed. Type-malformed fields had been fuzzed twice; nobody had fuzzed the characters inside a well-typed string. | Fixed in framework 1.3.0 (https://github.com/willdoucet/framework/pull/7): `_lib.one_line` replaces every unprintable character, `fmt_review_value` JSON-quotes unprintable values, and every reader's print goes through `_lib.encodable`. Found by `/review-implementation` of `workflow-rereview-utc` while fixing its other findings upstream; check added to REVIEW_CHECKLIST.md → Framework helpers. |
| 2026-09-23 | `frontend/vercel.json` rewrite + `/assets/(.*)` header rule (since M2) | A request for a `/assets/*.js` that does not exist gets `index.html` with `200`, `text/html` and `public, max-age=31536000, immutable`; Cloudflare reports `MISS` (cacheable). After a release an old tab's request for a previous-build chunk caches HTML under that name at the edge and in the browser, so a frontend rollback to that build cannot load it. The M8 RUNBOOK's chunk-load check triggers it, and the plan called the rollback "genuinely one command". Found by `/final-review`'s adversarial pass on M8 PR1a, confirmed live | RUNBOOK §5.1: Cloudflare Purge Everything and an empty-cache reload after a frontend rollback; the chunk-load check's expected signal corrected; TODOS.md P2 "A missing `/assets/*` chunk is answered with a year-cached HTML page" for the `vercel.json` fix; REVIEW_CHECKLIST → Vite → Operations |
| 2026-09-20 | `.agents/skills/review-implementation/SKILL.md` step 12, `.agents/skills/update-docs/SKILL.md` (framework 1.0.0 through 1.3.1) | Claude Code replaces `$0`, `$1` and `$2` in a skill body with the words typed after the slash command, before the agent reads it. `/review-implementation Scoped re-review of …` turned `awk '{i+=$1; d+=$2}'` into `awk '{i+=re-review; d+=of}'`: valid awk, exit 0, prints `0 0`, and the skill's next rule reads "small diff, adversarial subagent skipped, no log entry" whatever the diff size. `update-docs`' `awk '{print "A\t" $0}'` drops every untracked filename the same way. Nothing errors, so nothing surfaces it. | Caught in the scoped `/review-implementation` of `workflow-rereview-utc` only because the user had demanded the subagent pass; comparing the file on disk with the text the harness delivered showed the rewrite. The fix (`$(1)` in place of every bare positional, plus a lint over every skill file) shipped in framework 1.3.2 and was upgraded into todo-app on 2026-09-21; until then the workaround was to read a skill's shell snippets from the file on disk. Check added to REVIEW_CHECKLIST.md → Framework helpers → Operations. |
| 2026-09-23 | `todo_app_test`, shared by `api-test` and the integration suite | Run one after the other, each suite breaks the other's next run. After a visual run, the backend suite collapses once (84 failed / 264 errors). After a backend run, `drop_all` leaves `alembic_version` stamped at head with no model tables, so `api-test` skips its migration and the visual `globalSetup` fails with `unexpected login status 500`. The documented visual sequence did not mention either | Drop the three leftover tables and restart `api-test` before a visual run that follows the backend suite; re-run a backend suite that collapses right after a visual run. The rule is in "The visual-test stack and the integration suite share `todo_app_test`", and development-commands.md's visual block points at it. Found during M8 PR1a's `/ship` |
| 2026-09-24 | `app/gate_logging.py` (M8 PR1b, before merge); `app/auth/logging_utils.py:emit_log_line` (since M3) | The gate logged `logger.warning("host_gate", extra=payload)`. The app installs no logging config, so under uvicorn the root logger has no handler and Python's last-resort handler prints `%(message)s` alone: every rejection reached `fly logs` as a bare `host_gate`, and the diagnostics grep for `"event": "host_gate"` matched nothing. The M3 auth lines have printed only `auth.login` (failures) or nothing (INFO successes) all along. Every test read `caplog` record attributes, which carry the fields either way | The gate's message is the payload as one JSON object (`_emit`); tests format the record with `logging.Formatter("%(message)s")`, and one sends a `%0A` path to prove it stays one line. Auth lines: TODOS P2. Check added to REVIEW_CHECKLIST → FastAPI → Operations. Found by `/review-implementation` of M8 PR1b, confirmed with a real uvicorn |
| 2026-09-24 | `app/auth/service.py` `rotate_password` / `logout` vs `login`, `_refresh_case_a` (M8 PR1b, before merge) | The revoke `UPDATE refresh_tokens … WHERE revoked_at IS NULL` misses a row a concurrent login (which checked the old hash during argon2) or refresh inserts and commits after the statement began: READ COMMITTED takes the snapshot once, and the Case A row lock only re-checks rows already in it. The session outlived the rotation, and a Case A comment written in M3 claimed the opposite | `auth_session_issue` advisory lock: login and refresh take it shared before any row lock, `rotate_password` exclusive after hashing, to `logout()`'s commit. Three two-connection tests, each red without its lock. Check added to REVIEW_CHECKLIST → PostgreSQL. Found by the review's adversarial subagent, reproduced on the test database. `/final-review` found the same race in `logout()` (Sign out, since M3), the other revoke-everything path: it now takes the lock exclusive too, after the bearer check, and `test_logout_waits_for_a_sign_in_in_flight` is red without it. When one path gets a concurrency fix, grep for every path that writes the same predicate |
| 2026-09-24 | `app/gate_logging.py` `ip` (M8 PR1b, before merge) | Logged `request.client.host` as "the socket peer", but production runs uvicorn with `--proxy-headers --forwarded-allow-ips=*`, which sets it to the FIRST `X-Forwarded-For` entry, the one the sender writes: a direct-to-origin probe chose its own logged IP. The ASGI test transport skips uvicorn's middleware, so the test passed | Log the LAST `X-Forwarded-For` entry across every copy of the header (the hop Fly's proxy wrote, whether it appends or replaces), the socket peer only without one. The regression test wraps the app in `ProxyHeadersMiddleware(trusted_hosts="*")`. Check added to REVIEW_CHECKLIST → FastAPI. Found by the review's adversarial subagent. Superseded by the `Fly-Client-IP` row below: the last entry is Fly's own edge |
| 2026-09-24 | `.github/workflows/ops-check.yml` (M8 PR1b, first dispatch after the release) | `superfly/flyctl-actions/setup-flyctl` puts only a `flyctl` binary on PATH, and `infra/release-smoke.py` runs `fly`. The first run (36009722470) exited 2 on `[9]` and `[5]`: "`fly` is not on PATH". Nothing caught it before: `infra/tests` fakes every `fly` call, and every real run had been from a laptop where Homebrew installs both names | A step after setup links `$RUNNER_TEMP/flybin/fly` to `flyctl` and adds it to `$GITHUB_PATH`; `infra/tests/test_ops_check_workflow.py` runs that step on a PATH holding only `flyctl`. Check added to REVIEW_CHECKLIST → GitHub Actions. Found by Between the PRs step 10, the dispatch that exists to catch it (quickfix `ops-check-fly-path`) |
...
- To log who sent a request that went around the edge, use the header Fly's own proxy sets, `Fly-Client-IP`. No `X-Forwarded-For` entry works: the first is the sender's, and the last is Fly's edge (the app's anycast address), identical on every request. Whether Fly overwrites a client-sent `Fly-Client-IP` is unproven until the check in TODOS.md runs at the next release.
...
## A Fly host migration silently resets the volume's snapshot history

Fly can move a machine to a new host. It creates a **new volume in a new zone**, marks the old
one `pending_destroy`, and preserves the data — but the new volume starts with **zero snapshots**,
and `Scheduled snapshots: true` only means the daily cycle will begin, not that any restore point
exists. Nothing warns you, the app stays perfectly healthy, and the loss is invisible from
outside.

- **Rule:** treat "the volume id changed" as a backup incident, not a platform detail. After any
  host migration, confirm continuous backups are on and record when the first scheduled snapshot
  actually lands — the configured retention is not the recovery-point objective; the oldest
  existing snapshot is.
- `fly volumes list -a <app>` hides detached volumes. Use `--all` to see the outgoing one.
- Volume snapshot subcommands infer the app from the **current directory's `fly.toml`**. Running
  `fly volumes snapshots list <vol>` from `backend/` targets `mealy-app-prod` and fails with
  "volume does not belong to app". Always pass `-a mealy-app-prod-db` explicitly.
- Scheduled volume snapshots and `fly pg backup` (continuous, point-in-time) are **separate
  mechanisms**. Having the first configured says nothing about the second, which was found
  disabled.
- A liveness probe cannot detect this. `/healthz` was 200 and `/auth/status` returned
  `{"account_exists":true}` throughout — data was intact; only recoverability was gone.

Discovered 2026-09-12 while executing the M8 plan's assignment: `mealy-app-prod-db` had migrated
to a new host the previous night and production was running with no restore point at all. See
`.agents/plans/features/prod-launch-release/` item 12 and smoke check 9.
...
- A branch carries uncommitted `LESSONS.md` / `TODOS.md` rows from its plan reviews until `/ship` commits them, so `git merge origin/master` refuses when master touched the same doc. Save the branch's diff (`git diff -- <doc> > patch`), `git checkout -- <doc>`, merge, then `git apply --3way patch`; when both sides appended a table row after the same line, keep both, master's first. Done 2026-09-18 (UTC) on `workflow-rereview-utc` for #54's Bug Log row.
...
- **Run one after the other, each leaves the database in a state that breaks the other's next
  run.** Reproduced both ways on 2026-09-23 during M8 PR1a's `/ship`:
  - After a visual run, `todo_app_test` holds `api-test`'s seed. The next backend run fails
    with exactly **84 failed / 264 errors**, and the run after it passes (**915**). The
    session-end `drop_all` of the failing run is what cleans it up. So the 2026-09-09
    collapse was probably the seed too, not only the mid-migration overlap.
  - After a backend run, `drop_all` leaves `alembic_version` (stamped at head),
    `recipes_archived` and `food_items_archived`, none of which are in `Base.metadata`.
    `api-test`'s boot `alembic upgrade head` then has nothing to do, `users` does not exist,
    and Playwright's `globalSetup` fails with `unexpected login status 500`.
- **Rule:** before a visual run that follows the backend suite, drop those three tables and
  restart `api-test` so it migrates from base:
  ...
  then `docker-compose --profile visual-test restart api-test`. When the backend suite follows
  a visual run, expect one collapsed run and re-run it. CI never sees either case, because each
  job gets a fresh database.
...
### A timezone test must differ at every hour, and set `TZ` where the code runs

- "Under `TZ=Pacific/Kiritimati` the slug carries the UTC date" sounds airtight and is not:
  UTC+14 shares its calendar date with UTC for 10 hours of every day, so a date-only assert
  passes against the *old local-time code* 42% of the time — and always in a CI run that
  happens to start in that window. Compare a field the offset changes at every hour (the hour
  itself, or the whole timestamp within a few seconds of `datetime.now(timezone.utc)`).
- In Python, setting `os.environ["TZ"]` inside the test process does nothing until
  `time.tzset()` runs, and `tzset` leaks into every later test. Run the helper in a
  **subprocess** with `env={"TZ": …}` instead (`_repo.run(..., env=)`).
- Same family as "Parse API timestamps as UTC" above: a timezone assertion needs a negative
  control — confirm it fails against the code it is meant to catch.
- Found 2026-09-17 by `/plan-eng-review` of `workflow-rereview-utc`, in the plan's test list,
...
| 2026-09-11 | A documented break-glass expires silently once the thing it bypasses ships into every image. `cloudflare-state.md`'s "redeploy the previous image restores service and reopens the bypass" was true the day it was written, when the previous image predated the origin-lock code (PR #46). Every image since contains the gate, so rolling back to a previous image (`fly deploy --image`; flyctl has no `fly releases rollback`, see the 2026-09-12 row) no longer removes it, and every other lever fails closed by design — blanking `ORIGIN_VERIFY_SECRET` refuses to boot, blanking `PUBLIC_API_HOST` 421s everything. **Rule:** when a security control ships, re-read every break-glass that assumed its absence and date-stamp the answer. A fail-closed control needs a deliberate escape hatch, not the absence of itself in an older build. | recorded by /office-hours (M8 spec review) | branch prod-launch-release |
| 2026-09-11 | On a single-operator deployment, the standard restore-drill mitigation ("train at least two people; single-person ownership is a dangerous gap") is structurally unavailable — single-tenant-per-household means there is exactly one operator by design. The compensating control is not cross-training but the **observed numbers written into the artifact**: restore duration, snapshot age, row counts. The document, not the operator's memory, is what survives. Applies to every operational runbook here, not just the backup drill. | recorded by /office-hours | branch prod-launch-release |
| 2026-09-11 | Split runbooks by **type** (release / diagnose / drill), not by length. The field's advice is "keep runbooks uncomfortably short", but this repo's long, heavily annotated `cloudflare-state.md` and `r2-cutover-runbook.md` demonstrably paid for themselves during the origin-lock rollout — the ordered recovery steps and the clipboard-collision gotcha are what made a confusing failure diagnosable. Mixing types in one document is the real failure mode; depth within one type is an asset here. | recorded by /office-hours | branch prod-launch-release |
| 2026-09-12 | A backend rollback on Fly is `fly deploy --image <previous ref> --skip-release-command`, never a "release rollback" — flyctl v0.4.102 has no such command (`fly releases` only lists). The flag is mandatory across a migration boundary: the previous image's Alembic tree cannot locate the newer revision in `alembic_version`, so its `release_command` fails and the deploy aborts — a "code only, never schema" doctrine that still runs the release command fails on exactly the case it exists for. Run it from the checkout of the target commit, because `fly deploy --image` re-applies the *current* checkout's `fly.toml [env]`. | recorded by /plan-eng-review (M8) | branch prod-launch-release |
| 2026-09-12 | A `/healthz` field backed by a store (database or Redis) is populated by a background refresher with a sub-second timeout, never read inline in the handler — not even behind a TTL cache, because a cache miss that *hangs* blows Fly's 2-second probe and the machine is killed. Extends the 2026-04-23 shallow-probe decision to any enrichment of the health body. | recorded by /plan-eng-review (M8) | branch prod-launch-release |
| 2026-09-14 | A Celery `task_postrun` heartbeat is not "every successful task." Sixteen tasks exist here and only four are on `beat_schedule`. Unfiltered upsert plus "worker dead = every row stale" marks the worker dead the first time a one-shot task (`health_check`, `extract_recipe_from_url`, …) succeeds. Filter writer *and* reader to `entry["task"]` names from `beat_schedule`; key rows by Celery task name, not beat-entry key. | recorded by /plan-adversarial-review (M8) | branch prod-launch-release |
| 2026-09-14 | A health summary over several independent jobs must follow the **worst** job and show its **evidence** (each job's age), never the newest success and never a bare "all good". Summarizing four jobs as "ran N min ago" from the newest timestamp lets a dead job hide behind a healthy one — the signal lies in the reassuring direction. And a poll that cannot refresh keeps its last reading with a note, with staleness computed at serve time from the real timestamps, rather than flipping to "can't check": a status that flaps on every dropped poll or database wake trains people to ignore it. Both rules exist so green is never unearned. | recorded by /plan-design-review (M8) | branch prod-launch-release |
| 2026-09-14 | A GitHub Actions scheduled workflow is not a between-release watchdog on a **public** repository. GitHub disables scheduled workflows after 60 days with no repository activity; workflow *runs do not reset the timer* — only commits, PRs, and similar do. A passing daily cron does not keep itself alive. Monthly releases reset the clock; a 50-day gap since the last commit is a documented re-arm. This is the 103-day silent outage in miniature if left unnamed. | recorded by /plan-adversarial-review (M8) | branch prod-launch-release |
...
| 2026-09-17 | When an approval can be overtaken by later work, re-approval is **declared and targeted, never inferred from a diff**. Automatic "dismiss stale approvals on change" assumes the change author is untrusted and reviewers do not edit; here every review edits the plan by design, and the editor is the one who knows which earlier approval they touched. So the pattern that transfers is change control's impact statement: the review that makes the change names the reviews that must run again, and only those go stale. Its known weakness is a reviewer who forgets, so the declaration is mandatory even when it is "none", and it clears only when the named review really runs again (or the operator waives it, visibly — as agreed at office-hours; the waiver was deferred before 1.2.0 shipped, see the `/plan-eng-review` row of the same date below, so in 1.2.0 the named review running again is the only way) — never by another review vouching. | recorded by /office-hours | branch workflow-rereview-utc |
| 2026-09-17 | "Store UTC, display local" assumes a render layer between storage and the reader. Where the records *are* the display — committed text such as a date in a doc, a plan filename, a frontmatter field, read by agents on a laptop, in CI and in cloud sessions — there is nowhere to convert, and "display local" collapses into *storing* local. UTC everywhere is then the conventional rule applied correctly, not a deviation. Every date comes from a command (`date -u +%F`, `date -u +%FT%TZ`), never from what an agent believes today is; the one genuine display surface, the dashboard, only needs a label. | recorded by /office-hours | branch workflow-rereview-utc |
| 2026-09-17 | An escape hatch is a gate's fail-open surface, so it is built on evidence, not for symmetry. The operator waiver of a re-review demand was the only way a log line could turn a blocked review ok without that review looking; it was 40% of the reader, broke twice in spec review and once more in eng review ("any entry carrying `waives_ts` is set aside" hid a failure), and no recorded case needed it. Deferred: a demand clears only when the named review runs again, and a *scoped* re-run keeps a mistaken demand cheap. Before adding a bypass to a control, ask what it costs to take the honest path instead; if the answer is minutes, ship without the bypass. Two companions from the same review: every reader-side field gets a type guard, because the banner swallows exceptions and a blank banner is an open gate; and every free-text value that reaches a status line is flattened and truncated by one helper, including values that land *inside* the `Next:` line, not only on lines of their own. | recorded by /plan-eng-review (user-approved) | branch workflow-rereview-utc |
| 2026-09-17 | A rollback that fails open cannot discover after the revert what it opened: capture the blocked set while the new reader still exists, and put the list in the revert. A mandatory flag whose easy value is "none" is a reminder, not a check that the reviewer thought — the skill must force the per-prior-review question immediately before the log call, not only document the flag. Truncation belongs on the free-text fragment, never on the formatted status line, or the instruction that tells an agent what to do is what gets eaten. A "newest of N" field is not a substitute for the list of N when a later step must act on every item. | recorded by /plan-adversarial-review | branch workflow-rereview-utc |
| 2026-09-18 | An honor-system declaration gets a second reader, not a lie detector. The reviewer who forgets is the real case (four of four reversals in the evidence; none was a lie), so the next review audits the previous `none` against the plan's breadcrumbs and may declare the demand itself; the gate stays declared-never-inferred. A per-review answer checked on write was considered and skipped: it defeats copy-paste but makes nobody think. | recorded by /plan-eng-review (scoped re-run, user-approved) | branch workflow-rereview-utc |
| 2026-09-18 | A rollback to a reader that lacks a state must, before the revert, enumerate mechanically (`git branch -a --contains <sha>` lists exactly the branches that could hold the state; never memory) and convert that state into one the old reader gates on (`blocked` + `reason`), then list it in the revert. A list alone is display; the conversion is the gate. | recorded by /plan-eng-review (scoped re-run, user-approved) | branch workflow-rereview-utc |
| 2026-09-21 | A liveness signal's "no data yet" must expire, measured from something the server knows, such as the web process's start time. Otherwise it is the silent failure it exists to catch: "NULL means waiting, not stale" is the common way to avoid alarming on a fresh deploy, and M8's card would have sat gray forever on a worker that was dead at deploy time, which is how 2026-09-11 started. Two companions from the same review. An error flag stored in the store whose failure it reports cannot fire in that failure (database down, table missing), so either record it elsewhere or state the limit where the operator reads it. A threshold shown in a browser is measured on server time (`jobs.now` plus the time since the response arrived), never `Date.now()` against a server timestamp, or one device with a wrong clock sees a different state. | recorded by /plan-eng-review (M8, scoped re-review of item 15a; 1A and 2A user-approved) | branch prod-launch-release |
| 2026-09-21 | A grace period anchored to a process start is rearmed by every restart of that process, including the restart an operator does while answering the alarm. Name the clock and what resets it in the same place as the rule. And once a predicate is pinned (`stale` on `/healthz.jobs`), every other section points at it: a leftover sentence that re-derives the same fact ("minimum beat interval", "one schedule interval") is the sentence someone will implement. | recorded by /plan-adversarial-review (M8, scoped re-review of item 15a) | branch prod-launch-release |
| 2026-09-21 | Before writing the message for a state, ask what must have happened for that state's inputs to be true. M8's `write_error` is stamped only when a task reached SUCCESS and its upsert failed, so a row that carries it proves a run. "Every row is stale, so nothing has run" contradicted the flag that put the card in that state, and it would have been permanent in the one fault the state exists for: recorder broken, worker fine, every row stale after 3 h. Key such copy on evidence of absence (a stale row with no flag), not on the absence of fresh rows. Two companions from the same review. When a first-match ladder has an "unreadable" rung above a "stopped" rung, an outage both sides share never reaches "stopped", so say which side must still be working for each word. And the sweep for leftover sentences after pinning a predicate is a grep for the old strings across the plan and the test artifact, not a reread of the sections one remembers: the pass that wrote that rule missed one in each. | recorded by /plan-eng-review (M8, Eng review 5, scoped re-review of the adversarial pass; 1A user-approved) | branch prod-launch-release |
| 2026-09-21 | An interactive mockup is a second implementation of the state ladder, and it is the one an implementer copies. When a later review changes a predicate or a string, the mockup's embedded logic drifts exactly like a leftover sentence, so the sweep for old strings covers the mockup's fixtures and functions too, and the Design Source of Truth says which side wins until it is fixed. Give the mockup one fixture per *discriminating* case, not one per state: M8's had a single Waiting fixture (two jobs run, two not) that was the very case the adversarial pass found, no never-run-and-overdue row, and a row renderer keyed on `write_error` alone, which would have put red text under a green headline. Companion: a row's words follow that row's own values, never the card's state, because the rows are the headline's evidence. | recorded by /plan-design-review (M8, scoped re-review of item 15a after the adversarial pass and Eng review 5) | branch prod-launch-release |
| 2026-09-21 | A parenthetical in a failure-mode row can restate a rule the scenario cell no longer means. After M8's Eng review 5, the "all stale, one flag" row still showed the right outcome while its Handled cell said the reassuring line applies "only when some row is fresh" — the predicate just rejected, and the one that calls a live worker stopped for as long as every row is flagged. Grep the words that restrict a rule ("only when"), not only the sentence that was replaced. Companion: a flag that proves a run does not prove the worker is still up. All flags and all stale is also a worker that died inside the shortest 3× window; the rows are identical, the card is already red, and the cron has already failed. Do not add a string to split them, and do not let a diagram say that picture means the worker is fine. A NULL recorded success is not "nothing has run" when the flag is set: the flag is the SUCCESS whose write failed. | recorded by /plan-adversarial-review (M8, scoped re-review of Eng review 5, 1A) | branch prod-launch-release |
| 2026-09-21 | A remote probe's exit class follows **where** the failure happened, not which command returned non-zero. `infra/release-smoke.py` check 4 runs Python inside a web machine over `fly ssh`. A traceback from that interpreter (broker refused, a broken import) is evidence about production and is exit 1. Only a failure to *reach* the machine (flyctl missing, tunnel down, token rejected) is tooling, exit 2. Treating every non-zero `fly ssh` as tooling would have reported an Upstash outage as "fix your laptop", which is exactly the confusion the three exit classes exist to prevent. The same rule applies to any check that executes on the target. | recorded by /execute-plan (M8 PR1a) | branch prod-launch-release |
