...
| 2026-09-11 | A documented break-glass expires silently once the thing it bypasses ships into every image. `cloudflare-state.md`'s "redeploy the previous image restores service and reopens the bypass" was true the day it was written, when the previous image predated the origin-lock code (PR #46). Every image since contains the gate, so `fly releases rollback` no longer removes it, and every other lever fails closed by design — blanking `ORIGIN_VERIFY_SECRET` refuses to boot, blanking `PUBLIC_API_HOST` 421s everything. **Rule:** when a security control ships, re-read every break-glass that assumed its absence and date-stamp the answer. A fail-closed control needs a deliberate escape hatch, not the absence of itself in an older build. | recorded by /office-hours (M8 spec review) | branch prod-launch-release |
