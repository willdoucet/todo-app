...
| 2026-09-24 | `app/gate_logging.py` `ip` (M8 PR1b, before merge) | Logged `request.client.host` as "the socket peer", but production runs uvicorn with `--proxy-headers --forwarded-allow-ips=*`, which sets it to the FIRST `X-Forwarded-For` entry, the one the sender writes: a direct-to-origin probe chose its own logged IP. The ASGI test transport skips uvicorn's middleware, so the test passed | Log the LAST `X-Forwarded-For` entry across every copy of the header (the hop Fly's proxy wrote, whether it appends or replaces), the socket peer only without one. The regression test wraps the app in `ProxyHeadersMiddleware(trusted_hosts="*")`. Check added to REVIEW_CHECKLIST → FastAPI. Found by the review's adversarial subagent |
