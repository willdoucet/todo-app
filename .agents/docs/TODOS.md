...
## P2 — Cloudflare config-as-code or drift-detection for M8 runbook  ✅ DONE (M8 PR1a, 2026-09-23)
**Status:** ✅ DONE on `prod-launch-release` (PR: https://github.com/willdoucet/todo-app/pull/60). Shape (a) was chosen: `infra/cloudflare-drift.py` is a read-only diff of the live WAF rate-limit rule, the origin-lock Transform Rule (presence only), Browser Cache TTL, the Access apps and Bot Fight Mode against `infra/cloudflare-state.md`. It runs as a release step (`infra/RUNBOOK.md` §2 step 6), alongside a dashboard check that happens every release whether or not the script runs. Scheduling it in CI needs a CI-held token and is a v1.1 item (plan § Deferred). Kept as a record; safe to remove on the next TODOS.md prune.
...
## P2 — A missing `/assets/*` chunk is answered with a year-cached HTML page
**What:** Make a request for a `/assets/*` file that does not exist return an honest, uncached 404 instead of the app's `index.html`. In `frontend/vercel.json`, stop the catch-all rewrite from matching `/assets/` (for example `"source": "/((?!assets/).*)"`), and make sure the `/assets/(.*)` header rule's `public, max-age=31536000, immutable` does not land on that 404. Then extend `infra/release-smoke.py` check 7 to request a nonce `/assets/*.js` and fail on a 200 or a cacheable body, and relax RUNBOOK §5.1's purge from a required step to a precaution.
**Why:** Observed live 2026-09-23 by `/final-review` of M8 PR1a: `https://mealy.dev/assets/<missing>.js` answers `200`, `content-type: text/html`, `cache-control: public, max-age=31536000, immutable`, `cf-cache-status: MISS` (cacheable). After a release, an old tab that asks for one of the previous build's chunks stores HTML under that chunk's name, in Cloudflare and in that browser, for a year. A frontend rollback to the previous build then asks for exactly those names and cannot load them. RUNBOOK §2 step 7's chunk-load check triggers it on purpose. Until this is fixed, every frontend rollback depends on the operator remembering RUNBOOK §5.1's Cloudflare Purge Everything and an empty-cache reload, and a household browser that hit an old chunk stays broken after a rollback until its cache is cleared.
**Pros:** (a) The frontend rollback becomes the one-command reversible action the M8 plan first described; (b) a stale tab sees a real load error instead of HTML parsed as a module; (c) smoke check 7 then guards it on every release and daily.
**Cons:** (a) Needs a verified deploy: nothing here can show how Vercel stamps headers on a 404, and a 404 cached for a year would be the same failure in a new shape; (b) Vercel previews are not same-site with the API (TODOS P3), so the check has to happen on a production promotion, at a quiet hour.
**Context:** Pre-existing since M2's `vercel.json`; found while reviewing the M8 rollback doctrine (plan item 1, "Rollback, as two doctrines", edited in place by the final review). Where to start: change the rewrite, deploy, then `curl -sI https://mealy.dev/assets/<nonce>.js` and read the status, `content-type`, `cache-control` and `cf-cache-status` before promoting anything else. If the asset header still lands on the 404, move the immutable header off the path rule (Vercel's default for hashed static files is `max-age=0, must-revalidate`, so decide whether the year-long cache is worth keeping) or add a Cloudflare Cache Rule that bypasses cache for non-200 `/assets/*` responses and record it in `infra/cloudflare-state.md`.
**Effort:** S (human: ~2 h / CC: ~30 min, including a verified deploy)
**Priority:** P2
...
## P3 — Cloudflare config drift detection for M2 Access bypass + /auth/* rate-limit  ✅ SUPERSEDED (M8 PR1a, 2026-09-23)
**Status:** ✅ SUPERSEDED on `prod-launch-release` (PR: https://github.com/willdoucet/todo-app/pull/60) by the P2 drift-detection item above. The Access-bypass half is moot: both Access applications are gone (the bypass in M5, Application 1 at the M7 cutover). The rate-limit half is covered by `infra/cloudflare-drift.py`'s `waf-rate-limit` comparison and the RUNBOOK dashboard step. Two corrections to the text below: the live rule is **5 requests / 10 seconds / IP**, not 10 req/min, and the hostname is `api.mealy.dev`, not `api.todo.williamdoucet.dev` (`infra/cloudflare-state.md`). Kept as a record; safe to remove on the next TODOS.md prune.
