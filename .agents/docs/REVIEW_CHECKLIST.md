...
- A record the writer accepts is one the reader will count: when the reader orders one record against another (`>=` on a timestamp), the writer refuses a record dated so that the reader would ignore it. A stored record that never takes effect fails open, and silently.
...
- Every string a reader prints survives `print()`: JSON accepts a lone surrogate (`"\ud800"`), UTF-8 cannot encode one, and inside a fail-silent wrapper the `UnicodeEncodeError` is a blank banner. Free-text fragments are flattened and stripped of unprintable characters by one helper, and the print sink itself is guarded, so a fragment nobody routed through the helper still cannot blank the output.
...
- A regression test for a helper is run once against the release it is meant to catch (`git archive <tag> payload` into a scratch directory, the new test files copied over it) and fails there; a test that passes on the old code proves nothing.
- A sentence in a docstring, comment or changelog that states an invariant ("never", "only", "every") is pinned by a test that fails when it is false, or it is not written: one patch release carried three false ones (a bare string holding `"` beside "every bare comma separates two fields"; "a caller's `reason_at` is never taken"; "only a hand-appended line lacks a `ts`").
