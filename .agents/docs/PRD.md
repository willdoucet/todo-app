...
> `.agents/plans/features/prod-contract-freeze/prod-contract-freeze-plan-20260421-182714.md`.
...
| Password Recovery | No self-service password reset in v1. Operator rotates the shared password via CLI (`fly ssh console`; shipped in M8 as `app.cli.rotate_password`, procedure in [RUNBOOK §6](../../infra/RUNBOOK.md#6-rotate-the-household-password)) — the rotation also invalidates all outstanding refresh tokens AND any still-unexpired access JWTs via a server-side token/session version check. |
