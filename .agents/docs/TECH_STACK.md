...
| DNS / edge | Cloudflare | Proxied DNS for both hosts, WAF rate limit on `/auth/*`, origin-lock Transform Rule (sets the `X-Origin-Verify` header the host gate requires), Access Application 1 until M7's teardown; `infra/cloudflare-state.md` |
