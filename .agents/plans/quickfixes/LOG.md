...
- Tests: infra/tests/test_ops_check_workflow.py (4: the reproduction with the script's own runner, the workflow's step run as `bash -e`, a missing flyctl fails the step, step order); red before the fix, `python3 -m pytest infra/tests -q` 165 passed after; two mutation checks red then restored; the real-runner proof is a dispatch from the branch (`gh workflow run ops-check.yml --ref quickfix/ops-check-fly-path`), pending at record time
