...
> **Note**: [AGENTS.md](./AGENTS.md) is the native project rules file for Grok Build. Grok Build supports `CLAUDE.md` for compatibility, but prefers `AGENTS.md` when present. This file is retained primarily for Claude Code users and contains the original detailed behavioral system prompt.
...
For the complete reference (services, required environment variables, exact `docker-compose` commands for the full stack, frontend, migrations, testing (including visual regression), and host-side helper tests), see the dedicated runbook:

→ **[Development Commands](development-commands.md)**

This is the single source of truth and is also referenced from AGENTS.md.
