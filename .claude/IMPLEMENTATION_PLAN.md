...
3. **M3 `prod-auth-backend-foundation`** ✓ shipped 2026-05-03 (#28; ops follow-ups #29 web VM bump, #30 pool_pre_ping hotfix, #31 cookie-jar gitignore) — auth models + migrations; argon2 password hashing; `/auth/register` (gated by `HOUSEHOLD_ACCESS_KEY` + "no existing account"); `/auth/login`; `/auth/refresh` (60s rotation grace window); `/auth/logout`; `/auth/status`; refresh-token SHA-256 hash at rest; session-version column for immediate post-rotation access revocation. Plan: [`.claude/plans/features/prod-auth-backend-foundation/prod-auth-backend-foundation-plan-20260501-170804.md`](./plans/features/prod-auth-backend-foundation/prod-auth-backend-foundation-plan-20260501-170804.md). Beginner-friendly summary: [`todo-app-notes/DevOps/M3 Auth Backend - Plain English Summary.md`](../todo-app-notes/DevOps/M3%20Auth%20Backend%20-%20Plain%20English%20Summary.md).
4. **M4 `prod-auth-frontend-session`** ✓ shipped 2026-05-04 (#33) — centralized `frontend/src/lib/api.js` (all 28 existing `VITE_API_BASE_URL` callers migrated); unified `/auth` portal; in-memory access token (never `localStorage`/`sessionStorage`); boot-sequence refresh; `return_to` redirect; silent-refresh on 401 for long-running requests. Plan: [`.claude/plans/features/prod-auth-frontend-session/prod-auth-frontend-session-plan-20260503-185400.md`](./plans/features/prod-auth-frontend-session/prod-auth-frontend-session-plan-20260503-185400.md). Beginner-friendly summary: [`todo-app-notes/DevOps/M4 Auth Frontend - Plain English Summary.md`](../todo-app-notes/DevOps/M4%20Auth%20Frontend%20-%20Plain%20English%20Summary.md).
5. **M5 `prod-auth-enforcement`** ✓ shipped 2026-05-15 (PR1 #34 2026-05-06, PR2 #36 2026-05-15) — shipped in two PRs: (1) wrapped backend routes with the auth dependency + frontend route protection behind the edge gate, verified end-to-end; (2) removed `/plumbing-test*` endpoints + frontend plumbing shell, doc reconcile, M5 plain-English summary. **Cursor implementation review constraint (2026-05-07): the wrapping `protected` APIRouter cannot gate the `/uploads/*` StaticFiles mount, so CF Access (Application 1, the edge gate) stays load-bearing until M7 replaces the mount with R2 + auth-proxied uploads.** The vestigial CF `/plumbing-test` Bypass Application (Application 2) was removed 2026-05-14 in PR2, resolving a cursor-implementation-review informational finding (path-normalization variants could match the bypass selector at the edge while normalizing to a non-plumbing-test path at the origin); the broader CF Access teardown (Application 1) remains an M7 exit-checklist item. Backend deployed to Fly + frontend auto-deployed via Vercel git integration on the #36 merge; smoke checks passed 2026-05-15. Plan: [`.claude/plans/features/prod-auth-enforcement/prod-auth-enforcement-plan-20260504-213635.md`](./plans/features/prod-auth-enforcement/prod-auth-enforcement-plan-20260504-213635.md). Beginner-friendly summary: [`todo-app-notes/DevOps/Learning/M5 Auth Enforcement - Plain English Summary.md`](../todo-app-notes/DevOps/Learning/M5%20Auth%20Enforcement%20-%20Plain%20English%20Summary.md).
6. **M6 `prod-config-health`** ✓ subsumed 2026-05-18 — M4's 27-caller API-client centralization + M2's env-driven CORS made this empty by design (per plan). Post-M5 audit: every remaining `grep -r "localhost"` hit is non-production-critical (test code, the `url_safety.py` SSRF blocklist, the PR #35 HTTPS-upgrade guard, the prod-gated CORS dev default, the dev-only `docker-compose.yml`, the single M4 centralized fallback). One hardening shipped on `prod-config-health`: `apiBase.js` now throws on a production build with `VITE_API_BASE_URL` unset instead of silently falling back to localhost. No separate plan file. See [authoritative plan → M6 RESOLUTION](./plans/features/prod-contract-freeze/prod-contract-freeze-plan-20260421-182714.md).
