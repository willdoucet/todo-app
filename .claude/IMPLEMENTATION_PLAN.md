...
5. **M5 `prod-auth-enforcement`** — **ships in two PRs**: (1) wrap backend routes with auth dependency + frontend route protection behind the edge gate, verify end-to-end (✓ shipped 2026-05-06 via #34); (2) remove `/plumbing-test*` endpoints. **Cursor implementation review constraint (2026-05-07): do not remove the Cloudflare Access policy while `/uploads/*` is still a public StaticFiles mount.** Either ship M7's authenticated upload serving first or keep Cloudflare Access on the API host until that replacement lands. CF Access removal (originally PR2 Steps 2.3-2.5) is therefore deferred to M7's exit checklist; PR2 stays scoped to plumbing-test deletion + doc reconcile + the M5 plain-English summary. Plan: [`.claude/plans/features/prod-auth-enforcement/prod-auth-enforcement-plan-20260504-213635.md`](./plans/features/prod-auth-enforcement/prod-auth-enforcement-plan-20260504-213635.md). Beginner-friendly summary: [`todo-app-notes/DevOps/Learning/M5 Auth Enforcement - Plain English Summary.md`](../todo-app-notes/DevOps/Learning/M5%20Auth%20Enforcement%20-%20Plain%20English%20Summary.md).
