...
5. **M5 `prod-auth-enforcement`** — **ships in two PRs**: (1) wrap backend routes with auth dependency + frontend route protection behind the edge gate, verify end-to-end; (2) remove `/plumbing-test*` endpoints. **Cursor implementation review constraint (2026-05-07): do not remove the Cloudflare Access policy while `/uploads/*` is still a public StaticFiles mount.** Either ship M7's authenticated upload serving first or keep Cloudflare Access on the API host until that replacement lands.
